...
# utilizza l'equazione 5 del nostro paper
...
# utilizza l'equazione 4 del nostro paper
...
# il vicinato nel paper è indicato con Ns,j
...
# ritorna il punteggio medio dato dall'utente in generale
# indicato con r segnato v nel paper
def punteggioMedioUtente(utente):
	return utenti[utente]['media']

# Ritorna il punteggio dato dall'utente utente all'hotel hotel. Se sono presenti più voti dati da un utente allo stesso hotel, allora viene restituito quello medio
# indicato con rv,i nel paper
...
# la somiglianza è indicata con w(s,v) nel paper
# viene calcolata con il coefficiente di correlazione di Pearson
...
# utilizza l'equazione 9 del paper
...
# calcola tutti i valori binomiali, per ogni utente e per ogni hotel
...
# creazione di un dataset contenente tutti i voti dati nelle recensioni
# serve a velocizzare il calcolo della somiglianza	
for key,value in utenti.items():
	for k,v in value['hotelVotati'].items():
		if v[1]!=0:
			v[2]=round(v[0]/v[1],2)
